...
It also introduces key ML concepts: optimization, cost minimization, gradients, bias–variance, data diagnostics, regularization, and learning dynamics.
...
β1, ..., βn – coefficients/weights for each feature

x1, ..., xn – input feature values

y_hat – predicted output

Each coefficient βi tells you how much the prediction changes when xi increases by 1 unit, assuming all other features stay the same.
...
### Limitations:
- Requires matrix inversion → unstable with multicollinearity  
...
Cost: **O(n³)** where n = number of features  
...
| Independence | Errors are independent | Prevents inflated significance | Durbin–Watson |
...
High VIF → unstable coefficients.
