...
```text
J_elastic = MSE + α * [ l1_ratio * Σ |βj| + (1 - l1_ratio) * Σ (βj^2) ]
```

Where:

- `α` – overall regularization strength  
- `l1_ratio` – how much L1 vs L2 (between 0 and 1)
...
| Method       | Penalty         | Coefficients             | Best Use Case                               |
|-------------|-----------------|--------------------------|---------------------------------------------|
| Ridge       | L2 (βj²)        | Shrinks, rarely zero     | Multicollinearity, all features useful      |
| Lasso       | L1 (|βj|)       | Many exactly zero        | Feature selection, sparse models            |
| Elastic Net | L1 + L2         | Shrinks + some zero      | Correlated features + sparsity needed       |
...
## 6. Role of α (Regularization Strength)

The `α` (alpha) parameter controls how strong the penalty is:

- `α = 0` → equivalent to plain Linear Regression (no regularization)  
- Small `α` → light regularization (coefficients close to OLS)  
- Large `α` → strong regularization (coefficients heavily shrunk)

### 6.1 Effect of Too Small α

- Model behaves like unregularized Linear Regression  
- Risk of overfitting remains  

### 6.2 Effect of Too Large α

- Coefficients get overly small  
- Underfitting – model becomes too simple, misses patterns  
...
- Interpretation similar to Linear Regression:
  - “Holding other variables constant, a 1-unit increase in `xj` changes `y` by `βj` units on average.”  
...
- Features with non-zero coefficients are “selected” as important  
...
## 11. When Ridge & Lasso Work Well (and When They Don’t)
...
- Relationship between features and target is highly non-linear and you haven’t engineered good features  
- There are complex interactions that are not included  
- Data is extremely noisy and even strong regularization can’t save it  
